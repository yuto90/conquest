...
- 実装Issue: [#55](https://github.com/yuto90/conquest/issues/55)
- 親Issue: [#4](https://github.com/yuto90/conquest/issues/4)
- 関連Issue: [#5](https://github.com/yuto90/conquest/issues/5)（オンライン対戦。対象外）
- 依存: Issue #32 のCPU対CPU観戦モード（実装済み）、Issue #19 のテストマトリクス
- 調査基準: `origin/main` の `2d0460f65a4a3b935be6690e77199d7bcae88ddc`
- 状態: 2026-08-16設計。下記4件の確認が終わるまで実装に入らない
- 実装プラン: [同一端末2人対戦モード Implementation Plan](../plans/2026-08-16-local-two-player.md)

本書は同一端末2人対戦の実装境界を定義する。実装時はIssue #55、本書、`docs/game-rules.md`を仕様の正本とし、矛盾がある場合はIssue #55の明示要件を優先する。タスク分割とred-green手順は実装プランに従う。

## 確認待ち

実装は次の回答後に進める。本文の操作・UIは提案値で書いてある。

| # | 判断 | 提案 | 採用しない案 |
| --- | --- | --- | --- |
| 1 | 2人対戦の出兵操作 | 自軍島から目標へドラッグ | 1Pと同じ2タップだけ。タップにプレイヤーIDがなく、敵島が「相手の選択」と「自分の攻撃」の両方に読める |
| 2 | 画面の向き | 同じ向きの1画面 | 2P HUDの180度回転。一時停止・結果・設定・海図の変更が大きい |
| 3 | モード名 | 英語 `2P LOCAL`、日本語 `2人対戦` | `PLAY VS PLAYER` など長いラベル。280幅の1行2チップに収まらない |
| 4 | 一時停止 | 既存の右上ボタンを共有 | 2P専用ボタン。本陣は左上で、右上は両方から届く |
